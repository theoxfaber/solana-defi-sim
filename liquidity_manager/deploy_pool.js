...
async function main() {
  ...
  console.log(
    `║  Mode: ${IS_VERIFY ? 'VERIFICATION (dry-run)' : 'LIVE LOCALNET       '}            ║`,
  );
  console.log(`╚══════════════════════════════════════════════════╝\n`);
  ...
  const [allowlistPda] = PublicKey.findProgramAddressSync([Buffer.from('allowlist')], PROGRAM_ID);
  ...
  if (!IS_VERIFY) {
    ...
    for (let i = 0; i < childWallets.length; i++) {
      ...
      // Create ATA
      await getOrCreateAssociatedTokenAccount(connection, authority, mint, child.publicKey);
